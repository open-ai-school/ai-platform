...
const AI_OR_HUMAN_GAME_META = {
  id: "ai-or-human",
  nameKey: "aiOrHuman.title",
  descKey: "aiOrHuman.description",
  icon: "🤖",
  component: AIOrHumanGame,
  difficulty: "medium" as const,
  estimatedMinutes: 2,
  category: "knowledge" as GameCategory,
};

/* ─── Featured Game Definition ─── */
const FEATURED_GAME_META = {
  id: "neural-network",
  nameKey: "playground.gameNeuralName",
  descKey: "playground.gameNeuralDesc",
  icon: "🧬",
...
function buildGameList(t: DynamicTranslate): GameMeta[] {
  ...
  return [featured, aiOrHuman, ...miniGames];
}
...
export default function LabPage() {
  ...
  const filteredGames = useMemo(() => {
    const games = allGames.filter((g) => g.id !== FEATURED_GAME_META.id);
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
          ...
          >
            ...
            <div className="px-5 py-5 sm:px-6 sm:py-6 flex items-center gap-5">
              <span className="text-4xl sm:text-5xl shrink-0" style={{ animation: "pg-float 3s ease-in-out infinite" }}>{featuredGame.icon}</span>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-[var(--color-text-muted)] mt-0.5 line-clamp-1 font-mono">{featuredGame.desc}</p>
              </div>
